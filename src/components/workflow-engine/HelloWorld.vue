    ...
    <Form ref="formItem" :model="formItem" :rules="ruleItem" :label-width="80">
      ...
      <Row>
        <Col :xs="24" :sm="24" :md="24" :lg="24">
          <Button id="login_btn" shape="circle" type="primary" :loading="loading" @click.native="login('formItem')">
      ...
      <Col :xs="10" :sm="10" :md="10" :lg="10" offset="12">
        <router-link to="/regindex">没有账号？点此注册～</router-link>
      </Col>
      </Row>
    ...
    methods: {
      login (name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            if (that.verifyCode.toLowerCase() === that.formItem.code.toLowerCase()) {
              let instance = this.$ajax.create({
                // 用来将token放到header上
                headers: {'token': window.localStorage.getItem('token')}
              })
              instance.post(that.GLOBAL.serverPath + '/authentication/login', {
                loginName: that.formItem.account,
                loginPassword: that.formItem.password
              })
